...
public interface ShortAnswerQuestionEntityRepository extends JpaRepository<ShortAnswerQuestionEntity, Integer> {
     ...
     * @return 返回题目信息
     */
    @Query(value = "select * from short_answer_question where corr_chapter_id = ?1 and short_answer_question_id not in (select question_id from answer_situation where user_id = ?2 and question_type = ?3 )", nativeQuery = true)
    List<ShortAnswerQuestionEntity> findNotAnsweredShortAnswerQuestionEntitiesByChapter(int chapter_id, int user_id, String question_type);
